...
\begin{itemize}
  ...
  \item \textbf{Corpus finalisation.} Once judged, the corpus is finalised in one chain: canonicalise \(\to\) republish the browsable graph \(\to\) strict GAT input \(\to\) single-configuration CE No-Edge-Feats rolling-window training \(\to\) predictions, with the Predictions tab enabled automatically. The production backtest spans every rolling month of the corpus: \textbf{MRR 0.631, Hits@1 0.512, Hits@3 0.685, Hits@10 0.888 over 168 validation months} (2010-02 to 2024-01), ranking against \(\approx\)466 candidate entities per query (random \(\approx\)0.013) --- an evaluation seven times longer and an order of magnitude harder than the 23-month, \(\approx\)50-candidate 2019--2020 studies. Unseen-entity MRR of 0.681 confirms the inductive generalisation Fall 2025 designed for.
...
Summer 2026 & Alexandra Paiz, Pierre Pujol, Ruiwen Wang & \begin{itemize}
    ...
    \item Full 15-year FNSPID corpus through the three-LLM chain on the GPU VM at \$0 API cost; GAT predictions over 168 backtest months (MRR 0.631)
...
\begin{itemize}
  ...
  \item Predictions JSON: 168 monthly leaderboards for the full corpus (2010-02 to 2024-01, MRR 0.631, Hits@1 0.512, Hits@3 0.685, Hits@10 0.888, \(\approx\)466 candidates per query), 23 for the GPT-5.5 refined corpus (MRR 0.863), and live STOXX leaderboards.
