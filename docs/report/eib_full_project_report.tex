...
\begin{itemize}
  ...
  \item Added monthly \textbf{GAT retraining and predictions for the live corpora} (STOXX MRR 0.905 over its first backtest months).
  ...
  \item Reopened the two lineage model questions as directly comparable variants on the GPT-5.5 corpus: \textbf{GAT vs.\ GraphSAGE} (Spring 2025's architecture comparison) and \textbf{baseline vs.\ NOTEARS causal features} (Spring 2026's method, ported verbatim), delivered as a model toggle with a metric comparison table in the UI.
...
\begin{itemize}
  ...
  \item \textbf{Model-variant switcher.} Sources shipping several trained models get a Model toggle on the Predictions tab plus a comparison table (variant \(\times\) MRR / Hits@1 / Hits@3 / Hits@10 / backtest months); the leaderboard, scorecard and explainer all follow the selected variant.
  \item \textbf{Source lifecycle.} The superseded 2019--2020 FNSPID source was retired behind its \texttt{available} flag once the VM corpus covered it; the default source is now the live STOXX corpus.
\end{itemize}
...
\begin{itemize}
  \item Weeks of pre-refinement extraction had left junk entities and name variants baked into the immutable month snapshots. A one-off cleaner (\texttt{scripts/clean\_snapshots.py}) rewrote them in place: junk-pattern and generic-phrase removal, TF-IDF canonical merging with a numeric-literal guard, edge rewrite with weight summation, degree recompute (19 junk entities removed, 8 variant merges on the STOXX corpus).
  \item Once three month snapshots existed, the daily cron gained a GAT stage: month snapshots are converted to the trainer's CSV shape (one aggregate row per month, weights preserved), a single CE No-Edge-Feats configuration is retrained over all accumulated months, and a fresh predictions leaderboard is published. First STOXX backtest: \textbf{MRR 0.905} (few validation months so far; the number will regress toward the FNSPID range as the corpus grows).
...
Summer 2026 & Alexandra Paiz, Pierre Pujol, Ruiwen Wang & \begin{itemize}
    ...
    \item Live corpora (STOXX Europe 600, full S\&P 100) with news factor model and monthly GAT retrain, on a daily VM cron
...
\begin{itemize}
  ...
  \item Predictions JSON: 23 monthly leaderboards (2019--2020 corpus) + 83 monthly leaderboards (2009--2016 corpus, MRR 0.654) + live STOXX leaderboards (MRR 0.905).
