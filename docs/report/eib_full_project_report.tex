...
A fourth finding comes from repeating the architecture comparison on the young STOXX live corpus, where the ranking \emph{inverts}: GAT 0.885 vs.\ GraphSAGE 0.506 over its first five provisional backtest months. Encoder ranking is corpus-dependent --- attention wins on the small, sparse, title-only live graph while mean aggregation wins on the large refined corpus --- which is itself an argument for shipping the encoders as swappable variants rather than crowning one.
