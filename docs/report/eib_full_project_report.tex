...
\begin{itemize}
  ...
  \item Launched the \textbf{full 15-year FNSPID corpus run} (54{,}563 articles, 2009--2024) through the complete three-LLM chain on the team GPU VM at zero API cost, in parallel checkpointed lanes; finalised the first half (2009--2016) end to end --- canonicalisation, strict graph, CE GAT, live predictions at MRR 0.654 over 83 backtest months (Section~\ref{sec:vmpipe}).
...
\begin{itemize}
  ...
  \item \textbf{Half-corpus finalisation (22 Aug).} To deliver predictions early, the fully judged first half (2009--2016) was finalised on its own: canonicalise \(\to\) republish \(\to\) strict GAT input (9{,}918 canonical triplets) \(\to\) single-configuration CE No-Edge-Feats rolling-window training \(\to\) predictions. Result: \textbf{MRR 0.654, Hits@1 0.556, Hits@3 0.693 over 83 backtest months} (2010-02 to 2016-12) --- a deliberately harder, longer backtest than the 23-month 2019--2020 evaluation. The Predictions tab for the source went live the same night; the remaining half re-finalises over everything when it completes.
...
Summer 2026 & Alexandra Paiz, Pierre Pujol, Ruiwen Wang & \begin{itemize}
    ...
    \item Full 15-year FNSPID corpus through the three-LLM chain on the GPU VM at \$0 API cost; half-corpus GAT live (MRR 0.654 / 83 months)
...
\begin{itemize}
  ...
  \item Full-corpus VM run: judged + refined + canonicalised FNSPID chunks (2009--2016 finalised; remainder in flight), master extraction CSVs, canonical UI corpus, strict GAT input.
...
\begin{itemize}
  \item \emph{Resolved at \$0.} The full 15-year working corpus (2009--2024) is running through the local-Qwen three-LLM chain on the team VM (Section~\ref{sec:vmpipe}); 2009--2016 is finalised and live, the remainder completes within days of writing. The earlier \(\approx\)\$200 API estimate became unnecessary.
